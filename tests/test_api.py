...
class TestCurrenciesAPI:
    ...
    def test_create_currency_success(self, client):
        response = client.post("/currencies", json={"code": "USD", "full_name": "US Dollar", "sign": "$"})
    ...
    def test_create_currency_duplicate(self, client):
        ...
        response = client.post("/currencies", json={"code": "USD", "full_name": "US Dollar 2", "sign": "$"})
        assert response.status_code == status.HTTP_409_CONFLICT

    def test_create_currency_invalid_code(self, client):
        response = client.post("/currencies", json={"code": "US", "full_name": "US Dollar", "sign": "$"})
    ...
    def test_get_currencies_with_data(self, client):
        client.post("/currencies", json={"code": "USD", "full_name": "US Dollar", "sign": "$"})
        client.post("/currencies", json={"code": "EUR", "full_name": "Euro", "sign": "€"})
        response = client.get("/currencies")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert len(data) == 2
...
class TestExchangeRatesAPI:
    ...
    def test_create_exchange_rate_duplicate_pair(self, client):
        ...
        response = client.post("/exchangeRates", json={"base_currency_code": "USD", "target_currency_code": "EUR", "rate": 0.95})
        assert response.status_code == status.HTTP_409_CONFLICT
...
class TestExchangeAPI:
    def setup_currencies_and_rates(self, client):
        client.post("/currencies", json={"code": "USD", "full_name": "US Dollar", "sign": "$"})
        client.post("/currencies", json={"code": "EUR", "full_name": "Euro", "sign": "€"})
        client.post("/currencies", json={"code": "RUB", "full_name": "Russian Ruble", "sign": "₽"})
        client.post("/exchangeRates", json={"base_currency_code": "USD", "target_currency_code": "EUR", "rate": 0.92})
        client.post("/exchangeRates", json={"base_currency_code": "USD", "target_currency_code": "RUB", "rate": 92.50})
    ...
    def test_convert_no_rate_available(self, client):
        client.post("/currencies", json={"code": "USD", "full_name": "US Dollar", "sign": "$"})
        client.post("/currencies", json={"code": "GBP", "full_name": "British Pound", "sign": "£"})
