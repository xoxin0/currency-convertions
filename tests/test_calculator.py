...
class TestCalculatorDirectRate:
    ...
    def test_direct_rate_conversion(self, db_session, sample_currencies):
        ...
        crud.create_exchange_rate(
            db_session, ExchangeRateCreate(base_currency_code="USD", target_currency_code="EUR", rate=Decimal('0.92'))
        )
    ...
    def test_direct_rate_with_high_precision(self, db_session, sample_currencies):
        ...
        from app import crud
        crud.create_exchange_rate(
            db_session, ExchangeRateCreate(base_currency_code="USD", target_currency_code="EUR", rate=Decimal('0.921234'))
        )
        result, method = calculator.convert_currency(
...
class TestCalculatorCrossRate:
    ...
    def test_cross_rate_via_usd(self, db_session, sample_currencies):
        ...
        crud.create_exchange_rate(
            db_session, ExchangeRateCreate(base_currency_code="USD", target_currency_code="RUB", rate=Decimal('92.50'))
        )
...
class TestCalculatorEdgeCases:
    ...
    def test_decimal_rounding(self, db_session, sample_currencies):
        ...
        from app import crud
        crud.create_exchange_rate(
            db_session, ExchangeRateCreate(base_currency_code="USD", target_currency_code="EUR", rate=Decimal('0.9255'))
        )
        result, _ = calculator.convert_currency(db_session, "USD", "EUR", Decimal('100'))
        assert result == Decimal('92.55')

        db_session.query(crud.ExchangeRate).delete()
        crud.create_exchange_rate(
            db_session, ExchangeRateCreate(base_currency_code="USD", target_currency_code="EUR", rate=Decimal('0.92555'))
        )
        result, _ = calculator.convert_currency(db_session, "USD", "EUR", Decimal('100'))
