"""CRUD-операции для работы с БД. Здесь только взаимодействие с базой данных, без бизнес-логики. Бизнес-логика (конвертация) — в отдельном файле calculator.py."""
...
def get_all_exchange_rates(db: Session) -> List[ExchangeRate]:
    ...
    Note: joinedload() — это "жадная загрузка" (eager loading). Вместо N+1 запросов (сначала курсы, потом для каждого валюту) мы делаем один запрос с JOIN.
    """
...
def get_exchange_rate_by_pair(
    ...
    Args: db: Сессия БД base_code: Код базовой валюты (например, "USD") target_code: Код целевой валюты (например, "EUR")
...
def create_exchange_rate(
    ...
    Raises: ValueError: Если одна из валют не найдена IntegrityError: Если курс для такой пары уже существует
    """
...
def update_exchange_rate(
    db: Session, pair: str, # например "USDEUR"
    new_rate: Decimal
) -> Optional[ExchangeRate]:
    """Обновить существующий курс.
    Args: db: Сессия БД pair: Строка из 6 символов (код1 + код2), например "USDEUR" new_rate: Новое значение курса
